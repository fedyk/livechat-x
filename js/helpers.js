...
(function (app) {
    ...
    (function (helpers) {
        ...
        helpers.TypedEventEmitter = TypedEventEmitter;
        /** Abort Error */
        ...
        helpers.ErrorWithType = ErrorWithType;
        /**
        ...
        helpers.createInjector = createInjector;
        class LRUCache extends TypedEventEmitter {
            constructor(max) {
                super();
                this.max = max;
                this.cache = new Map();
        ...
        helpers.shallowEqual = shallowEqual;
        function indexBy(items, prop) {
            const result = {};
        ...
        helpers.indexBy = indexBy;
        function getChatRoute(chat, myProfileId) {
        ...
        helpers.getChatRoute = getChatRoute;
        function getActiveThread(chat) {
        ...
        helpers.getActiveThread = getActiveThread;
        function getRecentThread(chat) {
        ...
        helpers.getRecentThread = getRecentThread;
        function getPresentAgent(users) {
        ...
        helpers.getPresentAgent = getPresentAgent;
        function mergeChats(currentChatsById, incomingChatsById, chatIds, myProfileId) {
        ...
        helpers.mergeChats = mergeChats;
        function compareChats(chats, incomingChats) {
        ...
        helpers.compareChats = compareChats;
        function mergeChat(currentChat, incomingChat) {
        ...
        helpers.getChatLastMessage = getChatLastMessage;
        function getLastThread(chat) {
        ...
        helpers.getLastThread = getLastThread;
        function getIncompleteThreadIds(chat) {
            const threadIds = [];
            for (let i = 0; i < chat.threadIds.length; i++) {
                const threadId = chat.threadIds[i];
        ...
        helpers.getIncompleteThreadIds = getIncompleteThreadIds;
        function getThreadLastMessage(thread) {
            if (thread.messages.length > 0) {
                return thread.messages[thread.messages.length - 1];
            }
        }
        helpers.getThreadLastMessage = getThreadLastMessage;
        function mergeMessages(target, ...sources) {
        ...
        helpers.getChatRecipient = getChatRecipient;
        ...
        helpers.getChatMessages = getChatMessages;
        function sneakPeekToMessage(sneakPeek) {
            return {
                id: "sneak_peek",
                type: "sneak_peek",
                text: sneakPeek.text,
        ...
        helpers.sneakPeekToMessage = sneakPeekToMessage;
        ...
        helpers.stringifyMessage = stringifyMessage;
        function stringifyGeolocation(geolocation) {
            const parts = [];
            if (geolocation.city) {
        ...
        helpers.stringifyGeolocation = stringifyGeolocation;
